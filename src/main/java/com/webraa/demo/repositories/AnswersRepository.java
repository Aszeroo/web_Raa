...
public interface AnswersRepository extends DataTablesRepository<Answers, String> {
    ...
    @Query(value = "select topic_id,topic_name,sum(ans_yes) ans_yes,sum(ans_no) ans_no from \n" +
            ...
            "join t_answers b on a.question_id=b.question_id join t_topics c on a.topic_id=c.topic_id where b.create_by = :userName and b.round_id = :roundId) a group by topic_id,topic_name order by topic_id", nativeQuery = true)
    List<String> summaryAnswer(@Param("userName") String userName,@Param("roundId") String roundId);
